...
function NewDisc(props){
        ...
        .then(userResp=> {
            ...
            .then(res=>{
                ...
                async function retreivePlaces(){
                    return new Promise((resolve, reject)=>{
                        axios.get("/places/getPlaces/" + userResp.data[0].college)
                        .then(resp=>{
    ...
    function handleSubmit(e){
        ...
        if(subject===""||topic===""||duration===""||discussionMaterial==="" ||place===""||maxattend===""){
            ...
            .then(resp=>{
                axios.get("/topics/" + props.match.params.id.slice(3,) + "/" + subject)
                .then(res=>{
                    let topic = res.data[res.data.length-1];
                    let new_topic_id=topic._id;
                    axios.post("/discussions/add", {
                        place:place,
                        start_time:date,
                        duration: duration,
