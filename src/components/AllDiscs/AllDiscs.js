...
function AllDiscs(props){
    ...
    function handleJoinGroup(discId){
        axios.put("/discussions/addAttendee/" + discId + "/" + userId);
    }
    function retreiveAllPending(){
        return new Promise((resolve, reject)=>{
            axios.get("/users/getId/"+userId)
            .then(resp=>{
                axios.get("/places/getPlaces/" + resp.data[0].college)
                .then(res=>{
                    res.data.forEach(place=>{
                        places.push(place.place);
                        discussions.push([]);
                        axios.get("/discussions/getPending/" + place._id)
                        .then(pendingDiscs=>{
                            pendingDiscs.data.forEach((disc, index)=>{
                                axios.get("/topics/"+disc.topic)
                                .then(top=>{
